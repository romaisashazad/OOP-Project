...
int main() {
    ...
    while (window.isOpen()) {
        ...
        sf::Vector2f position=rectangle.getPosition();//recent position of shape on the screen
        sf::Vector2f size=rectangle.getSize();

        if(position.x > 800){ //change in x-axis
            rectangle.setPosition(-size.x, position.y); //check if current pos is greater than the max width of screen
        } else if(position.x + size.x < 0){
            rectangle.setPosition(800, position.y); //readjust to appear from the right most corner of the screen i.e x=800
        }

        if(position.y>600){ //change in y-axis
            rectangle.setPosition(position.x, -size.y);
        } else if(position.y + size.y < 0){
            rectangle.setPosition(position.x, 600);
        }
